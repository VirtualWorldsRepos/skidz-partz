...
function themenu() {
	...
	if (is_client($client)) {
		if ($op == "client_home") {
			...
			$client_opt = "<a href=\"index.php?name=$module_name&op=client_home\">"._MYADS."</a>";
		}
	} else {
		$client_opt = "<a href=\"index.php?name=$module_name&op=client\">"._CLIENTLOGIN."</a>";
	}
	Open_Table();
	echo "<center><b>"._ADSMENU."</b><br><br>[ <a href=\"index.php?name=$module_name\">"._MAINPAGE."</a> | <a href=\"index.php?name=$module_name&op=sitestats\">"._SITESTATS."</a> | <a href=\"index.php?name=$module_name&op=terms\">"._TERMS."</a> | <a href=\"index.php?name=$module_name&op=plans\">"._PLANSPRICES."</a> | $client_opt ]</center>";
	Close_Table();
}
...
function sitestats() {
    global $module_name, $prefix, $db, $user_prefix, $site_url, $sitename;
	$url = $site_url;	
...
function client() {
	...
	if (is_client($client)) {
		Header("Location: index.php?name=$module_name&op=client_home");
	} else {
		include("header.php");
		title("$sitename: "._ADSYSTEM."");
		Open_Table();
		echo "<center><font class=\"title\"><b>"._CLIENTLOGIN."</b></font></center><br>";
		echo "<form method=\"POST\" action=\"index.php?name=$module_name\"><table border=\"0\" align=\"center\" cellpadding=\"3\"><tr>";
...
function client_logout() {
	...
	$client = "";
	setcookie("client");
	Header("Location: index.php?name=$module_name&op=client");
	die();
}
	...
	if (!is_client($client)) {
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			echo "<td align=\"center\">".$row['name']."</td>"
				...
				."<td align=\"center\"><a href=\"index.php?name=$module_name&op=client_report&cid=$cid&bid=$bid\"><img src=\"images/edit.gif\" border=\"0\" alt=\""._EMAILSTATS."\" title=\""._EMAILSTATS."\"></a>  <a href=\"index.php?name=$module_name&op=view_banner&cid=$cid&bid=$bid\"><img src=\"images/view.gif\" border=\"0\" alt=\""._VIEWBANNER."\" title=\""._VIEWBANNER."\"></a></td><tr>";
		}
		echo "</table>";
		$row = $db->sql_fetchrow($db->sql_query("SELECT * FROM ".$prefix."_banner_clients WHERE cid='$cid'"));
		...
		echo "</table><br><br><center>[ <a href=\"index.php?name=$module_name&op=client_logout\">"._LOGOUT."</a> ]</center>";
		Close_Table();
		themenu();
		include("footer.php");
	}
	...
	if (!is_client($client)) {
		...
		if ($cid != $client_id) {
			...
			echo "<td align=\"center\">".$row['name']."</td>"
				...
				."</table><br><br>"
				."[ <a href=\"index.php?name=$module_name&op=client_report&cid=$cid&bid=$bid\">"._EMAILSTATS."</a> | <a href=\"index.php?name=$module_name&op=logout\">"._LOGOUT."</a> ]";
			Close_Table();
			themenu();
			include("footer.php");
		}
