...
if(!isset($pollID)) {
    ...
    if (isset($tid)) {
		Header("Location: index.php?name=$module_name"); // SecurityReason Fix 2005 - sp3x -> Now should be OK.
	}
	include ("header.php");
	Open_Table();
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		if ($pollID == $plid) {
			...
			echo "<img src='images/arrow.gif' border='0'>&nbsp;<a href='index.php?name=$module_name&op=results&pollID=$plid$r_options'>$pltitle</a> ($plvoters "._LVOTES.")<br><br>";
		}
	}
	echo "<div align=\"right\"><a href='index.php?name=$module_name'><b>"._MOREPOLLS."</b></a></div>";
	Close_Table();
	cookiedecode($user);
...
function pollMain($pollID) {
	...
	if(!isset($pollID))
	$pollID = 1;
	$boxContent .= "<form action=\"index.php?name=$module_name\" method=\"post\">";
	...
	$boxContent .= "<br><font class=\"content\"><a href=\"index.php?name=$module_name&amp;op=results&amp;pollID=$pollID&amp;mode=".$userinfo['umode']."&amp;order=".$userinfo['uoder']."&amp;thold=".$userinfo['thold']."\"><b>"._RESULTS."</b></a><br><a href=\"index.php?name=$module_name\"><b>"._POLLS."</b></a><br>";
...
function pollList() {
	...
	for ($count = 0; $count < count($resultArray); $count++) {
		...
		echo "<strong><big>&middot;</big></strong>&nbsp;<a href=\"index.php?name=$module_name&amp;pollID=$id\">$pollTitle</a> ";
		if (is_admin($admin)) {
			$editing = " - <a href=\"".$admin_file.".php?op=polledit&amp;pollID=$id\">Edit</a>";
		} else {
			$editing = "";
		}
		echo "(<a href=\"index.php?name=$module_name&amp;op=results&amp;pollID=$id$r_options\">"._RESULTS."</a> - $sum "._LVOTES."$editing)<br>\n";
		$sum = 0;
	}
	echo "</td></tr></table>";
	echo "<table border=\"0\" cellpadding=\"8\"><tr><td>";
	...
	for ($count = 0; $count < count($resultArray2); $count++) {
		...
		echo "(<a href=\"index.php?name=$module_name&amp;op=results&amp;pollID=$id$r_options\">"._RESULTS."</a> - $sum "._LVOTES."$editing)<br>\n"
		.""._ATTACHEDTOARTICLE." <a href=\"index.php?name=News&amp;file=article&amp;sid=$sid$r_options\">$title</a><br><br>\n";
		$sum = "";
	}
	echo "</td></tr></table>";
	Close_Table();
...
function pollResults($pollID) {
	...
	echo "[ <a href=\"index.php?name=$module_name&amp;pollID=$booth\">"._VOTING."</a> | "
	."<a href=\"index.php?name=$module_name\">"._OTHERPOLLS."</a> ] $article";
	if (is_admin($admin)) {
