...
function modone() {
	...
	if ($comnum != 0) {
		if(((isset($admin)) && ($moderate == 1)) || ($moderate==2)) echo "<form action=\"index.php?name=$module_name&amp;file=comments\" method=\"post\">";
	}
}
...
function modtwo($tid, $score, $reason) {
	...
	$comnum = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_poll_desc WHERE pollID='$pollsid' AND comments!='0'"));
	if ($comnum != 0) {
		$whoisath = $db->sql_fetchrow($db->sql_query("SELECT firstname, lastname FROM ".$prefix."_pollcomments WHERE tid='$tid'"));
		cookiedecode($user);
		if((((isset($admin)) && ($moderate == 1)) || ($moderate == 2)) && ($user)) 
		{
			if (strtolower($cookie[1]) == strtolower($whoisath['firstname']) && strtolower($cookie[2]) == strtolower($whoisath['lastname'])) 
			{
...
function navbar($pollID, $title, $thold, $mode, $order) {
	...
	if($title) {
		...
		if(is_user($user)) {
			echo "<a href=\"index.php?name=Your_Account&amp;op=editcomm\"><font color=\"$textcolor1\">"._CONFIGURE."</font></a>";
		} else {
			echo "<a href=\"index.php?name=Your_Account\"><font color=\"$textcolor1\">"._LOGINCREATE."</font></a>";
		}
		if(($count==1)) {
	...
	if (($pollcomm) AND ($mode != "nocomments")) {
		if ($anonpost==1 OR (isset($admin) AND is_admin($admin)) OR is_user($user)) {
			if (!isset($pid)) { $pid = 0; }
			echo "<form action=\"index.php?name=$module_name&amp;file=comments\" method=\"post\">"
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
   global $datetime, $user, $cookie, $bgcolor1, $reasons, $sl_firstname, $sl_lastname, $anonpost, $commentlimit, $prefix, $module_name, $db, $userinfo, $user_prefix;
	$comments = 0;
                  cookiedecode($user);
                  getusrinfo($user);
                  if (!isset($mode) OR empty($mode)) {
	...
	$tid = intval($tid);
	$result = $db->sql_query("SELECT tid, pid, pollID, date, firstname, lastname, email, host_name, subject, comment, score, reason from ".$prefix."_pollcomments where pid = '$tid' order by date, tid");
	if ($mode == 'nested') {
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			$r_date = $row['date'];
			$r_firstname = filter($row['firstname'], "nohtml");
			$r_lastname = filter($row['lastname'], "nohtml");
			...
			if($r_score >= $thold) {
				...
				$comments++;
				if (!eregi("[a-z0-9]",$r_firstname)) $r_firstname = $sl_firstname;
				if (!eregi("[a-z0-9]",$r_lastname)) $r_lastname = $sl_lastname;
				...
				if ($r_email) {
					...
					echo "<br>"._BY." <a href=\"mailto:$r_email\">$r_firstname $r_lastname</a> <font class=content><b>($r_email)</b></font> "._ON." $datetime";
				} else {
					echo "<p><b>$r_subject</b> <font class=content>";
					if($userinfo['noscore'] == 0) {
						echo "("._SCORE." $r_score";
					...
					echo "<br>"._BY." $r_firstname $r_lastname "._ON." $datetime";
				}
				if ($r_firstname != $sl_firstname && $r_lastname != $sl_lastname) {
					$row2 = $db->sql_fetchrow($db->sql_query("SELECT user_id FROM ".$user_prefix."_users WHERE firstname='$r_firstname' AND lastname='$r_lastname'"));
					$r_uid = intval($row2['user_id']);
					echo "<br>(<a href=\"index.php?name=Your_Account&op=userinfo&amp;firstname=$r_firstname&amp;lastname=$r_lastname\">"._USERINFO."</a> )";
                }
				$row_url = $db->sql_fetchrow($db->sql_query("SELECT user_website FROM ".$prefix."_users WHERE firstname='$r_firstname' AND lastname='$r_lastname'"));
				...
				if((isset($userinfo['commentmax'])) && (strlen($r_comment) > $userinfo['commentmax'])) echo substr($r_comment, 0, $userinfo['commentmax'])."<br><br><b><a href=\"index.php?name=$module_name&file=comments&pollID=$r_pollID&tid=$r_tid&mode=$mode&order=$order&thold=$thold\">"._READREST."</a></b>";
				elseif(strlen($r_comment) > $commentlimit) echo substr("$r_comment", 0, $commentlimit)."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;pollID=$r_pollID&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._READREST."</a></b>";
				else echo $r_comment;
				echo "</td></tr></table><br><p>";
				if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
					echo "<font class=content color=\"$bgcolor2\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$r_tid&amp;pollID=$r_pollID&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
				}
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				if ($r_firstname != $sl_firstname && $r_lastname != $sl_lastname) {
					$row3 = $db->sql_fetchrow($db->sql_query("SELECT user_id FROM ".$prefix."_users WHERE firstname='$r_firstname' AND lastname='$r_lastname'"));
					$ruid = intval($row3['user_id']);
					echo "<BR>(<a href=\"index.php?name=Your_Account&amp;op=userinfo&amp;firstname=$r_firstname&amp;lastname=$r_lastname\">"._USERINFO."</a>) ";
				}
				$row_url2 = $db->sql_fetchrow($db->sql_query("SELECT user_website FROM ".$prefix."_users WHERE firstname='$r_firstname' AND lastname='$r_lastname'"));
				...
				echo "</td></tr></table><br><p><font class=content color=\"$bgcolor2\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$r_tid&amp;pollID=$r_pollID&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
				modtwo($r_tid, $r_score, $r_reason);
				echo " ]</font><p>";
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				if (!eregi("[a-z0-9]",$r_subject)) $r_subject = "["._NOSUBJECT."]";
				formatTimestamp($r_date);
				echo "<li><font class=\"content\"><a href=\"index.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;pollID=$r_pollID&amp;pid=$r_pid&amp;mode=$mode&amp;order=$order&amp;thold=$thold#$r_tid\">$r_subject</a> "._BY." $r_firstname $r_lastname "._ON." $datetime</font><br>";
...
function DisplayBabies ($tid, $level=0, $dummy=0) {
	global $userinfo, $datetime, $sl_firstname, $sl_lastname, $prefix, $db, $module_name;
	$comments = 0;
	$tid = intval($tid);
	$result = $db->sql_query("SELECT tid, pid, pollID, date, firstname, lastname, email, host_name, subject, comment, score, reason from ".$prefix."_pollcomments where pid = '$tid' order by date, tid");
	while($row = $db->sql_fetchrow($result)) {
		...
		if (!eregi("[a-z0-9]",$r_subject)) { $r_subject = "["._NOSUBJECT."]"; }
		formatTimestamp($r_date);
		echo "<a href=\"index.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">$r_subject</a><font class=\"content\"> "._BY." $r_firstname $r_lastname "._ON." $datetime<br>";
...
function DisplayTopic ($pollID, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	global $hr, $user, $datetime, $cookie, $userinfo, $admin, $commentlimit, $sl_firstname, $sl_lastname, $reasons, $anonpost, $foot1, $foot2, $foot3, $foot4, $prefix, $module_name, $db, $admin_file, $user_prefix;
	...
	$tid = intval($tid);
	$q = "select tid, pid, pollID, date, firstname, lastname, email, host_name, subject, comment, score, reason from ".$prefix."_pollcomments where pollID='$pollID' and pid='$pid'";
	if(!empty($thold)) {
		$q .= " and score>='$thold'";
	} else {
		$q .= " and score>='0'";
	...
	while ($count_times < $num_tid) {
		...
		$date = $row_q['date'];
		$c_firstname = filter($row_q['firstname'], "nohtml");
		$c_lastname = filter($row_q['lastname'], "nohtml");
		...
		$karma = $db->sql_fetchrow($db->sql_query("SELECT karma FROM ".$user_prefix."_users WHERE firstname='$c_firstname' AND lastname='$c_lastname'"));
		...
		if (empty($c_firstname)) { $c_firstname = $sl_firstname; }
		if (empty($c_lastname)) { $c_lastname = $sl_lastname; }
		...
		if ($email) {
			...
			echo "<br>"._BY." <a href=\"mailto:$email\">$c_firstname $c_lastname</a> <b>($email)</b> "._ON." $datetime";
		} else {
			echo "<p><b>$subject</b> <font class=content>";
			if($userinfo['noscore'] == 0) {
				echo "("._SCORE." $score";
			...
			echo "<br>"._BY." $c_firstname $c_lastname "._ON." $datetime";
		}
		...
		if (is_active("Journal")) {
			...
			if (!empty($jid) AND isset($jid)) {
				$journal = " | <a href=\"index.php?name=Journal&amp;file=display&amp;jid=$jid\">"._JOURNAL."</a>";
			} else {
				$journal = "";
			}
		}*/
		if ($c_firstname != $sl_firstname && $c_lastname != $sl_lastname) {
			$row2 = $db->sql_fetchrow($db->sql_query("SELECT user_id FROM ".$user_prefix."_users WHERE firstname='$c_firstname' AND lastname='$c_lastname'"));
			$r_uid = intval($row2['user_id']);
			echo "<br>(<a href=\"index.php?name=Your_Account&op=userinfo&firstname=$c_firstname&lastname=$c_lastname\">"._USERINFO."</a> ";
            echo "$journal) ";
		}
		$row_url = $db->sql_fetchrow($db->sql_query("SELECT user_website FROM ".$prefix."_users WHERE firstname='$c_firstname' AND lastname='$c_lastname'"));
		$url = filter($row_url['user_website'], "nohtml");
		if ($url != "http://" AND !empty($url) AND stripos_clone($url, "http://")) { echo "<a href=\"$url\" target=\"new\">$url</a> "; }
		...
		echo "</font></td></tr><tr><td>";
		if((isset($userinfo['commentmax'])) && (strlen($comment) > $userinfo['commentmax'])) echo substr("$comment", 0, $userinfo['commentmax'])."<br><br><b><a href=\"index.php?name=$module_name&file=comments&pollID=$r_pollID&tid=$r_tid&mode=$mode&order=$order&thold=$thold\">"._READREST."</a></b>";
		elseif(strlen($comment) > $commentlimit) echo substr("$comment", 0, $commentlimit)."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;pollID=$pollID&tid=$tid&mode=$mode&order=$order&thold=$thold\">"._READREST."</a></b>";
		else echo $comment;
		echo "</td></tr></table><br><p>";
		if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
			echo "<font class=\"content\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$tid&amp;pollID=$pollID&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
		}
		if ($pid != 0) {
			$row4 = $db->sql_fetchrow($db->sql_query("SELECT pid from ".$prefix."_pollcomments where tid='$pid'"));
			$erin = intval($row4['pid']);
			echo "| <a href=\"index.php?name=$module_name&amp;file=comments&amp;pollID=$pollID&amp;pid=$erin&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._PARENT."</a>";
		}
...
function singlecomment($tid, $pollID, $mode, $order, $thold) {
	...
	global $userinfo, $user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $anonpost, $admin, $sl_firstname, $sl_lastname, $prefix, $db, $module_name;
                  cookiedecode($user);
                  getusrinfo($user);
                  if (!isset($mode) OR empty($mode)) {
                    if(isset($userinfo['umode'])) {
	...
	$tid = intval($tid);
	$pollID = intval($pollID);
	$row = $db->sql_fetchrow($db->sql_query("SELECT date, firstname, lastname, email, subject, comment, score, reason from ".$prefix."_pollcomments where tid='$tid' and pollID='$pollID'"));
	$date = $row['date'];
	...
	if(empty($firstname)) $firstname = $sl_firstname;
	if(empty($lastname)) $lastname = $sl_lastname;
	if(empty($subject)) $subject = "["._NOSUBJECT."]";
	modone();
	echo "<table width=99% border=0><tr bgcolor=\"$bgcolor1\"><td width=500>";
	formatTimestamp($date);
	if($email) echo "<p><b>$subject</b> <font class=content>("._SCORE." $score)<br>"._BY." <a href=\"mailto:$email\"><font color=\"$bgcolor2\">$firstname $lastname</font></a> <font class=content><b>($email)</b></font> "._ON." $datetime";
	else echo "<p><b>$subject</b> <font class=content>("._SCORE." $score)<br>"._BY." $firstname $lastname "._ON." $datetime";
	echo "</td></tr><tr><td>$comment</td></tr></table><br><p><font class=content color=\"$bgcolor2\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&pid=$tid&pollID=$pollID&mode=$mode&order=$order&thold=$thold\">"._REPLY."</a> | <a href=\"index.php?name=$module_name&amp;pollID=$pollID\">"._ROOT."</a>";
	modtwo($tid, $score, $reason);
	echo " ]";
	modthree($pollID, $mode, $order, $thold);
...
function reply ($pid, $pollID, $mode, $order, $thold) {
	include_once("header.php");
	global $userinfo, $user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $AllowableHTML, $sl_firstname, $sl_lastname, $prefix, $anonpost, $module_name, $db, $wysiwyg_editor;
                  cookiedecode($user);
                  getusrinfo($user);
                  if (!isset($mode) OR empty($mode)) {
                    if(isset($userinfo['umode'])) {
	...
	if ($anonpost == 0 AND !is_user($user)) {
		...
		if($pid!=0) {
			list($date, $firstname, $lastname, $email, $subject, $comment, $score) = $db->sql_fetchrow($db->sql_query("select date, firstname, lastname, email, subject, comment, score from ".$prefix."_pollcomments where tid='$pid'"));
		...
		$titlebar = "<b>$subject</b>";
		if(empty($firstname)) $firstname = $sl_firstname;
		if(empty($lastname)) $lastname = $sl_firstname;
		if(empty($subject)) $subject = "["._NOSUBJECT."]";
		formatTimestamp($date);
		Open_Table();
		echo "<center><font class=\"title\"><b>"._SURVEYCOM."</b></font></center>";
		...
		if (is_user($user)) {
			cookiedecode($user);
			echo "<font class=\"content\"><a href=\"index.php?name=Your_Account\">$cookie[1] $cookie[2]</a> [ <a href=\"index.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font>";
		} else {
			echo "<font class=\"content\">$sl_firstname $sl_lastname</font>";
			$xanonpost=1;
		}
...
function replyPreview ($pid, $pollID, $subject, $comment, $xanonpost, $mode, $order, $thold) {
	include_once("header.php");
	global $userinfo, $user, $cookie, $AllowableHTML, $sl_firstname, $sl_lastname, $module_name, $wysiwyg_editor;
	cookiedecode($user);
                  getusrinfo($user);
	...
	if (is_user($user)) 
	{
		echo $cookie[1] . "&nbsp;" . $cookie[2];
	} else {
		echo $sl_firstname . "&nbsp;" . $sl_lastname;
	}
	...
	if (is_user($user)) {
		echo "<font class=\"content\"><a href=\"index.php?name=Your_Account\">".$cookie[1]."&nbsp;".$cookie[2]."</a> <font class=\"content\">[ <a href=\"index.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font>";
	} else {
		echo "<font class=\"content\">".$sl_firstname."&nbsp;".$sl_lastname."</font>";
	}
	echo "<br><br><font class=\"content\"><B>"._SUBJECT.":</B></FONT><BR>"
  ...
  if(isset($userinfo['thold'])) {
	...
	if ($fake == 1) {
		if ((($anonpost == 0) AND (is_user($user))) OR ($anonpost == 1)) {
			if (is_user($user)) {
				$krow = $db->sql_fetchrow($db->sql_query("SELECT karma FROM ".$user_prefix."_users WHERE firstname='$firstname' AND lastname='$lastname'"));
				$koptions = "&mode=".$mode."&order=".$order."&thold=".$thold;
				if ($krow['karma'] == 2) {
					$db->sql_query("INSERT INTO ".$prefix."_pollcomments_moderated VALUES (NULL, '$pid', '$pollID', now(), '$firstname', '$lastname', '$email', '$url', '$ip', '$subject', '$comment', '$score', '0', '0')");
					include_once("header.php");
					title(""._MODERATEDTITLE."");
					Open_Table();
					echo "<center>"._COMMENTMODERATED."";
					echo "<br><br><a href=\"index.php?name=$module_name&op=results&pollID=$pollID$koptions\">"._MODERATEDTITLE."</a>";
					Close_Table();
					include_once("footer.php");
					die();
				} elseif ($krow['karma'] == 3) {
					Header("Location: index.php?name=$module_name&op=results&pollID=$pollID$koptions");
					die();
				}
			}
			$db->sql_query("insert into ".$prefix."_pollcomments values (NULL, '$pid', '$pollID', now(), '$firstname', '$lastname', '$email', '$url', '$ip', '$subject', '$comment', '$score', '0', '0')");
	...
	if ($pollcomm == 1) {
	  ...
	  Header("Location: index.php?name=$module_name&op=results&pollID=$pollID$options");
	}
}
...
switch($op) {
	...
	Header("Location: index.php?name=$module_name&op=results&pollID=$pollID");
	break;
	...
	if ((isset($tid)) && (!isset($pid))) {
		...
		Header("Location: index.php?name=$module_name&op=results&pollID=$pollID&mode=$mode&order=$order&thold=$thold");
	} else {
		if(!isset($pid)) $pid=0;
		if(!isset($tid)) $tid=0;
		DisplayTopic($pollID, $pid, $tid, $mode, $order, $thold);
