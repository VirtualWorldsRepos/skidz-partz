...
function select_month() {
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		if ($month != $thismonth) {
			$year = $getdate[1];
			echo "<li><a href=\"index.php?name=$module_name&amp;sa=show_month&amp;year=$year&amp;month=$getdate[2]&amp;month_l=$month\">$month, $year</a>";
			$thismonth = $month;
		}
	}
	echo "</ul>"
	...
	if(is_active("Search"))
	{
	   echo "<form action=\"index.php?name=Search\" method=\"post\">"
	       ."<input type=\"text\" name=\"query\" size=\"30\">&nbsp;"
	       ."<input type=\"submit\" value=\""._SEARCH."\">"
	       ."</form><br><br>";
	}
	echo "[ <a href=\"index.php?name=$module_name&amp;sa=show_all\">"._SHOWALLSTORIES."</a> ]</center>";
	Close_Table();
	include("footer.php");
}
...
function show_month($year, $month, $month_l) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		$time = explode(" ", $time);
		$actions = "<a href=\"index.php?name=News&amp;file=print&amp;sid=$sid\"><img src=\"images/print.gif\" border=0 alt=\""._PRINTER."\" title=\""._PRINTER."\" width=\"16\" height=\"11\"></a>&nbsp;<a href=\"index.php?name=News&amp;file=friend&amp;op=FriendSend&amp;sid=$sid\"><img src=\"images/friend.gif\" border=0 alt=\""._FRIEND."\" title=\""._FRIEND."\" width=\"16\" height=\"11\"></a>";
		if ($score != 0) {
		...
		if ($catid == 0) {
			$title = "<a href=\"index.php?name=News&amp;file=article&amp;sid=$sid$r_options\">$title</a>";
		} elseif ($catid != 0) {
			...
			$title = "<a href=\"index.php?name=News&amp;file=categories&amp;op=newindex&amp;catid=$catid\"><i>$cat_title</i></a>: <a href=\"index.php?name=News&amp;file=article&amp;sid=$sid$r_options\">$title</a>";
		}
		if ($multilingual == 1) {
			if (empty($alanguage)) {
				$alanguage = $language;
	...
	echo "[ <a href=\"index.php?name=$module_name\">"._ARCHIVESINDEX."</a> | <a href=\"index.php?name=$module_name&amp;sa=show_all\">"._SHOWALLSTORIES."</a> ]</center>";
	Close_Table();
	include("footer.php");
}
...
function show_all($min) {
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		$ratings = intval($row['ratings']);
		$time = explode(" ", $time);
		$actions = "<a href=\"index.php?name=News&amp;file=print&amp;sid=$sid\"><img src=\"images/print.gif\" border=0 alt=\""._PRINTER."\" title=\""._PRINTER."\" width=\"15\" height=\"11\"></a>&nbsp;<a href=\"index.php?name=News&amp;file=friend&amp;op=FriendSend&amp;sid=$sid\"><img src=\"images/friend.gif\" border=0 alt=\""._FRIEND."\" title=\""._FRIEND."\" width=\"15\" height=\"11\"></a>";
		if ($score != 0) {
			$rated = substr($score / $ratings, 0, 4);
		} else {
			$rated = 0;
	...
	if (($numrows > 250) AND ($min == 0)) {
		$min = $min+250;
		$a++;
		echo "<center>[ <a href=\"index.php?name=$module_name&amp;sa=show_all&amp;min=$min\">"._NEXTPAGE."</a> ]</center><br>";
	}
	if (($numrows > 250) AND ($min >= 250) AND ($a != 1)) {
		$pmin = $min-250;
		$min = $min+250;
		$a++;
		echo "<center>[ <a href=\"index.php?name=$module_name&amp;sa=show_all&amp;min=$pmin\">"._PREVIOUSPAGE."</a> | <a href=\"index.php?name=$module_name&amp;sa=show_all&amp;min=$min\">"._NEXTPAGE."</a> ]</center><br>";
	}
	if (($numrows <= 250) AND ($a != 1) AND ($min != 0)) {
		$pmin = $min-250;
		echo "<center>[ <a href=\"index.php?name=$module_name&amp;sa=show_all&amp;min=$pmin\">"._PREVIOUSPAGE."</a> ]</center><br>";
	}
	...
	echo "[ <a href=\"index.php?name=$module_name\">"._ARCHIVESINDEX."</a> ]</center>";
	Close_Table();
	include("footer.php");
}
