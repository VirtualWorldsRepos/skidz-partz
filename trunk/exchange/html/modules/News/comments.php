...
function modone() {
	...
	if ($comnum != 0) {
		if(((isset($admin)) AND ($moderate == 1)) || ($moderate==2)) echo "<form action=\"index.php?name=$module_name&file=comments\" method=\"post\">";
	}
}
...
function navbar($sid, $title, $thold, $mode, $order) {
	...
	if($title) {
		...
		if ($count > 0 AND is_active("Search")) {
			echo " | <a href='index.php?name=Search&type=comments&sid=$sid'>"._SEARCHDIS."</a>";
		}
		echo "</font></td></tr>\n";
	}
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
   global $datetime, $user, $cookie, $bgcolor1, $reasons, $sl_firstname, $anonpost, $commentlimit, $prefix, $textcolor2, $db, $module_name, $user_prefix, $userinfo, $cookie;
	$tid = intval($tid);
   $comments = 0;
   cookiedecode($user);
   getusrinfo($user);
	...
	if ($mode == 'nested') {
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				$comments++;
				if (!eregi("[a-z0-9]",$r_name)) $r_name = $sl_firstname;
				...
				$options .= "&order=".$order;
				$options .= "&thold=".$thold;
				if((isset($userinfo['commentmax'])) AND (strlen($r_comment) > $userinfo['commentmax'])) echo substr("$r_comment", 0, $userinfo['commentmax'])."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">"._READREST."</a></b>";
				elseif(strlen($r_comment) > $commentlimit) echo substr("$r_comment", 0, $commentlimit)."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">"._READREST."</a></b>";
				else echo $r_comment;
				echo "</td></tr></table><br><br>";
				if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
				echo "<font class=\"content\" color=\"$textcolor2\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$r_tid&amp;sid=$r_sid$options\">"._REPLY."</a>";
				}
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				echo "</td></tr></table><br><br>";
				if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
					echo "<font class=\"content\" color=\"$textcolor2\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$r_tid&amp;sid=$r_sid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
				}
				modtwo($r_tid, $r_score, $r_reason);
				echo " ]</font><br><br>";
		...
		while ($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				$options .= "&order=".$order;
				$options .= "&thold=".$thold;
				echo "<li><font class=\"content\" color=\"$textcolor2\"><a href=\"index.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;sid=$r_sid&amp;pid=$r_pid$options#$r_tid\">$r_subject</a> "._BY." $r_name "._ON." $datetime</font><br>";
...
function DisplayBabies ($tid, $level=0, $dummy=0) {
	global $datetime, $sl_firstname, $prefix, $db, $module_name, $userinfo, $cookie, $user;
	cookiedecode($user);
	getusrinfo($user);
	$tid = intval($tid);
	$comments = 0;
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		formatTimestamp($r_date);
	        ...
	        $options .= "&order=".$order;
	        $options .= "&thold=".$thold;
		echo "<a href=\"index.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid$options\">$r_subject</a></font><font class=\"content\"> "._BY." $r_name "._ON." $datetime<br>";
...
function DisplayTopic ($sid, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	global $title, $bgcolor1, $bgcolor2, $bgcolor3, $hr, $user, $datetime, $cookie, $admin, $commentlimit, $sl_firstname, $reasons, $anonpost, $foot1, $foot2, $foot3, $foot4, $prefix, $acomm, $articlecomm, $db, $module_name, $site_url, $admin_file, $user_prefix, $userinfo;
	$sid = intval($sid);
	$pid = intval($pid);
	$tid = intval($tid);
	...
	while ($count_times < $num_tid) {
		...
		if (empty($c_name)) { $c_name = $sl_firstname; }
		...
		if ($c_name != $sl_firstname) {
			$row2 = $db->sql_fetchrow($db->sql_query("SELECT user_id FROM ".$user_prefix."_users WHERE username='$c_name'"));
			$r_uid = intval($row2['user_id']);
			echo "<br>(<a href=\"index.php?name=Your_Account&amp;op=userinfo&amp;username=$c_name\">"._USERINFO."</a> ";
            echo "$journal) ";
		}
		...
		echo "</font></td></tr><tr><td>";
	        ...
	        $options .= "&order=".$order;
	        $options .= "&thold=".$thold;
		if((isset($userinfo['commentmax'])) AND (strlen($comment) > $userinfo['commentmax'])) echo substr($comment, 0, $userinfo['commentmax'])."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">"._READREST."</a></b>";
		elseif(strlen($comment) > $commentlimit) echo substr($comment, 0, $commentlimit)."<br><br><b><a href=\"index.php?name=$module_name&amp;file=comments&amp;sid=$sid&tid=$tid$options\">"._READREST."</a></b>";
		else echo $comment;
		echo "</td></tr></table><br><br>";
		if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
			echo "<font class=\"content\"> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$tid&amp;sid=$sid$options\">"._REPLY."</a>";
		}
		if ($pid != 0) {
			$row4 = $db->sql_fetchrow($db->sql_query("SELECT pid FROM ".$prefix."_comments WHERE tid='$pid'"));
			$erin = intval($row4['pid']);
			echo " | <a href=\"index.php?name=$module_name&amp;file=comments&amp;sid=$sid&amp;pid=$erin$options\">"._PARENT."</a>";
		}
...
function singlecomment($tid, $sid, $mode, $order, $thold) {
	...
	if(empty($name)) $name = $sl_firstname;
	if(empty($subject)) $subject = "["._NOSUBJECT."]";
	modone();
	Open_Table();
	echo "<table width=\"99%\" border=\"0\"><tr bgcolor=\"$bgcolor1\"><td width=\"500\">";
	...
	if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
	        ...
	        $options .= "&order=".$order;
	        $options .= "&thold=".$thold;
		echo "<font class=content> [ <a href=\"index.php?name=$module_name&amp;file=comments&amp;op=Reply&amp;pid=$tid&amp;sid=$sid$options\">"._REPLY."</a> | <a href=\"index.php?name=$module_name&amp;file=article&amp;sid=$sid$options\">"._ROOT."</a>";
	}
	modtwo($tid, $score, $reason);
	echo " ]";
	modthree($sid, $mode, $order, $thold);
...
function reply($pid, $sid, $mode, $order, $thold) {
	...
	include("header.php");
	global $prefix, $module_name, $user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $db, $anonpost, $sl_firstname, $admin;
	cookiedecode($user);
	getusrinfo($user);
	$sid = intval($sid);
	$pid = intval($pid);
	if ($anonpost == 0 AND !is_user($user)) {
		...
		if ($pid != 0) {
			$row = $db->sql_fetchrow($db->sql_query("SELECT date, firstname, lastname, email, subject, comment, score FROM ".$prefix."_comments WHERE tid='$pid'"));
			$date = $row['date'];
			...
			$row2 = $db->sql_fetchrow($db->sql_query("SELECT time, title, hometext, bodytext, firstname, lastname, notes FROM ".$prefix."_stories WHERE sid='".$sid."'"));
			$date = $row2['time'];
		...
		if (!empty($email)) {
			echo "<br>"._BY." <a href=\"mailto:$email\">$firstname $lastname</a> <font class=\"content\"><b>($email)</b></font> "._ON." $datetime";
		} else {
			echo "<br>"._BY." $firstname $lastname"._ON." $datetime";
		}
		...
		if (is_user($user)) {
			cookiedecode($user);
			echo "<a href=\"index.php?name=Your_Account\">$cookie[1] $cookie[2]</a> <font class=\"content\">[ <a href=\"index.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font><br><br>";
		} else {
			echo "<font class=\"content\">$sl_firstname $sl_lastname";
			echo " [ <a href=\"index.php?name=Your_Account\">"._NEWUSER."</a> ]<br><br>";
		}
...
function replyPreview ($pid, $sid, $subject, $comment, $xanonpost, $mode, $order, $thold) {
  global $module_name, $user, $cookie, $AllowableHTML, $sl_firstname, $anonpost, $userinfo;
	include("header.php");
	cookiedecode($user);
	getusrinfo($user);
	$sid = intval($sid);
	...
	echo "<br>";
	Open_Table();
	echo "<form action=\"index.php?name=$module_name&amp;file=comments\" method=\"post\"><font class=\"option\"><b>"._YOURNAME.":</b></font> ";
	if (is_user($user)) {
		echo "<a href=\"index.php?name=Your_Account\">$cookie[1]</a> <font class=\"content\">[ <a href=\"index.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font><br><br>";
	} else {
		echo "<font class=\"content\">$sl_firstname<br><br>";
	}
	echo "<font class=\"option\"><b>"._SUBJECT.":</b></font><br>"
...
function CreateTopic ($xanonpost, $subject, $comment, $pid, $sid, $host_name, $mode, $order, $thold) {
	global $module_name, $user, $userinfo, $EditedMessage, $cookie, $AllowableHTML, $user_prefix, $prefix, $anonpost, $articlecomm, $db, $sitename;
	cookiedecode($user);
	getusrinfo($user);
	$sid = intval($sid);
	$pid = intval($pid);
		...
		if (($anonpost == 0 AND is_user($user)) OR $anonpost == 1) {
			if (is_user($user)) {
				...
				if ($krow['karma'] == 2) {
					...
					echo "<br><br><a href=\"index.php?name=$module_name&file=article&sid=$sid$koptions\">"._MODERATEDRETURN."</a>";
					Close_Table();
					include("footer.php");
					die();
				} elseif ($krow['karma'] == 3) {
					Header("Location: index.php?name=$module_name&file=article&sid=$sid$koptions");
					die();
				}
			}
	...
	$options .= "&order=".$order;
	$options .= "&thold=".$thold;
	Header("Location: index.php?name=$module_name&file=article&sid=$sid$options");
}
