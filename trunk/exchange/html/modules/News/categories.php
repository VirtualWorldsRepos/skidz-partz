...
function theindex($catid) {
	global $storyhome, $httpref, $httprefmax, $topicname, $topicimage, $topictext, $datetime, $user, $cookie, $site_url, $prefix, $multilingual, $currentlang, $db, $articlecomm, $module_name, $userinfo;
	...
	$result = $db->sql_query("SELECT sid, aid, aid2, title, time, hometext, bodytext, comments, counter, topic, firstname, lastname, notes, acomm, score, ratings FROM ".$prefix."_stories where catid='$catid' $querylang ORDER BY sid DESC limit $storynum");
	...
	if ($httpref==1) {
		...
		if ($referer=="" OR ereg("unknown", $referer) OR eregi($site_url,$referer)) {
		} else {
