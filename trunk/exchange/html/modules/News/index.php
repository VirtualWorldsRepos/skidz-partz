...
{
   global $db, $storyhome, $topicname, $topicimage, $topictext, $datetime, $user, $cookie, $site_url, $prefix, $multilingual, $currentlang, $articlecomm, $sitename, $user_news, $userinfo;
	...
	$result = $db->sql_query("SELECT sid, catid, aid, aid2, title, time, hometext, bodytext, comments, counter, topic, firstname, lastname, notes, acomm, score, ratings FROM ".$prefix."_stories $qdb $querylang ORDER BY sid DESC limit $storynum");
	while ($row = $db->sql_fetchrow($result)) {
		$s_sid = intval($row['sid']);
		$catid = intval($row['catid']);
		$aid = filter($row['aid'], "nohtml");
		$aid2 = filter($row['aid2'], "nohtml");
		...
		$story_link = "<a href=\"index.php?name=News&amp;file=article&amp;sid=$s_sid$r_options\">";
		$morelink = "(";
		...
		if ($catid != 0) {
			...
			$title = "<a href=\"index.php?name=News&amp;file=categories&amp;op=newindex&amp;catid=$catid\"><font class=\"storycat\">$title1</font></a>: $title";
			$morelink .= " | <a href=\"index.php?name=News&amp;file=categories&amp;op=newindex&amp;catid=$catid\">$title1</a>";
		}
		if ($score != 0) {
		...
		themeindex($aid, $aid2, $firstname, $lastname, $datetime, $title, $counter, $topic, $hometext, $notes, $morelink, $topicname, $topicimage, $topictext);
	}
	include("footer.php");
}
...
function rate_article($sid, $score, $random_num="0", $gfx_check) {
	...
	if (isset($random_num)) {
		...
		if (extension_loaded("gd") AND $code != $gfx_check AND $gfx_chk != 0) {
			...
			echo "<center><a href=\"index.php?name=$module_name&file=article&sid=$sid$r_options\"><b>".$row['title']."</b></a><br>"._ARTICLERATING.": <img src=\"images/articles/stars-$score.gif\" border=\"0\" alt=\"$score/5\" title=\"$score/5\"> ($score/5)<br><br>";
			...
			if ($score) {
				...
				if ($num != 0) {
					Header("Location: index.php?name=News&op=rate_complete&sid=$sid&rated=1");
					die();
				}
				if (isset($ratecookie)) {
				...
				if ($a == 1) {
					...
					update_points(7);
					Header("Location: index.php?name=News&op=rate_complete&sid=$sid&score=$score");
				}
			} else {
				include("header.php");
				title("$sitename: "._ARTICLERATING."");
		...
		if (extension_loaded("gd") AND $gfx_chk != 0 ) {
			...
			$gfx_check = "$code";
			Header("Location: index.php?name=$module_name&op=rate_article&sid=$sid&score=$score&random_num=$random_num");
		}
	}
}
...
function rate_complete($sid, $rated=0, $score) {
	...
	if ($rated == 0) {
		$row = $db->sql_fetchrow($db->sql_query("SELECT title FROM ".$prefix."_stories WHERE sid='$sid'"));
		$row['title'] = filter($row['title'], "nohtml");
		echo "<center><a href=\"index.php?name=$module_name&file=article&sid=$sid$r_options\"><b>".$row['title']."</b></a><br>"._YOURATEDARTICLE.": <img src=\"images/articles/stars-$score.gif\" border=\"0\" alt=\"$score/5\" title=\"$score/5\"> ($score/5)<br><br>";
		echo "<center>"._THANKSVOTEARTICLE."<br><br>"
		."[ <a href=\"index.php?name=$module_name&amp;file=article&amp;sid=$sid$r_options\">"._BACKTOARTICLEPAGE."</a> ]</center>";
	} elseif ($rated == 1) {
