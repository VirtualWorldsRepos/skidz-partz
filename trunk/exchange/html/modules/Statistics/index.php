...
function Stats_Main() {
	...
	."[ <a href=\"index.php?name=$module_name&op=Stats\">"._VIEWDETAILED."</a> ]</center>";
	Close_Table();
	echo "<br><br>";
	...
	echo "<tr><td><img src=\"modules/$module_name/images/mac.gif\" border=\"0\" alt=\"\">&nbsp;Mac OS:</td><td><img src=\"themes/$ThemeSel/images/leftbar.gif\" height=\"$l_size[1]\" width=\"$l_size[0]\" Alt=\"Mac OS\"><img src=\"themes/$ThemeSel/images/mainbar.gif\" Alt=\"Mac - PPC\" height=\"$m_size[1]\" width=", $mac[1] * 2, "><img src=\"themes/$ThemeSel/images/rightbar.gif\" height=\"$r_size[1]\" width=\"$r_size[0]\" Alt=\"Mac OS\"> $mac[1] % ($mac[0])</td></tr>\n";
	...
	$unum = $db->sql_numrows($db->sql_query("select user_id from ".$user_prefix."_users")) - 1;
	...
	if (is_active("Marketplace")) {//MARKETPLACE
		$links = $db->sql_numrows($db->sql_query("select * from ".$prefix."_marketplace_items"));
		$cat = $db->sql_numrows($db->sql_query("select * from ".$prefix."_marketplace_categories"));
	}
	Open_Table2();
	echo "<table cellspacing=\"0\" cellpadding=\"2\" border=\"0\" align=\"center\"><tr><td colspan=\"2\">\n";
	...
	if (is_active("Marketplace")) {
		echo "<tr><td><img src=\"modules/$module_name/images/topics.gif\" border=\"0\" alt=\"\">&nbsp;"._MARKETPLACEINLINKS."</td><td><b>".$links."</b></td></tr>\n";
		echo "<tr><td><img src=\"modules/$module_name/images/sections.gif\" border=\"0\" alt=\"\">&nbsp;"._MARKETPLACECAT."</td><td><b>".$cat."</b></td></tr>\n";
	}
	echo "</table>\n";
	Close_Table2();
	Close_Table();
...
function Stats($total) {
	...
	."[ <a href=\"index.php?name=$module_name\">"._RETURNBASICSTATS."</a> ]</center>";
...
function YearlyStats($year){
	...
	echo "<BR>";
	echo "<center>[ <a href=\"index.php?name=$module_name\">"._BACKTOMAIN."</a> | <a href=\"index.php?name=$module_name&amp;op=Stats\">"._BACKTODETSTATS."</a> ]</center>";
	Close_Table();
	include ("footer.php");
}
...
function showYearStats($nowyear){
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		if ($year != $nowyear) {
			echo "<a href=\"index.php?name=$module_name&amp;op=YearlyStats&amp;year=$year\">$year</a>";
		} else {
			echo "$year";
		}
		echo "</td><td>";
...
function showMonthStats($nowyear,$nowmonth){
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		echo "<tr bgcolor=\"$bgcolor1\"><td>";
		if ($month != $nowmonth) {
			echo "<a href=\"index.php?name=$module_name&amp;op=MonthlyStats&amp;year=$nowyear&amp;month=$month\" class=\"hover_orange\">";
			getmonth($month);
			echo "</a>";
		} else {
			getmonth($month);
...
function showDailyStats($year,$month,$nowdate){
	...
	while($row = $db->sql_fetchrow($result)) {
		...
		echo "<tr bgcolor=\"$bgcolor1\"><td>";
		if ($date != $nowdate) {
			echo "<a href=\"index.php?name=$module_name&amp;op=DailyStats&amp;year=$year&amp;month=$month&amp;date=$date\" class=\"hover_orange\">";
			echo $date;
			echo "</a>";
		} else {
			echo $date;
