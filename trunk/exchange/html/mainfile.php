...
if(!$dbname) {
    die("<br><br><center><img src=images/logo.gif><br><br><b>There seems that Smeego isn't installed yet.<br>(The values in config.php file are the default ones)<br><br>You can proceed with the <a href='./install/index.php'>web installation</a> now.</center></b>");
}
...
$paypal_url = filter($row['paypal_url'], "nohtml");
...
function is_admin($admin) 
{
	...
	$aid2 = $admin[1];
    $pwd = $admin[2];
    $aid = substr(addslashes($aid), 0, 31);
	$aid2 = substr(addslashes($aid2), 0, 31);
    if (!empty($aid) && !empty($aid2) && !empty($pwd)) {
        global $prefix, $db;
        $sql = "SELECT pwd FROM ".$prefix."_authors WHERE aid='".$aid."' AND  aid2='".$aid2."'";
...
function update_points($id) 
{
  ...
  if (is_user($user)) 
  {
	...
	{
      $cookie = cookiedecode($user);
      $firstname = trim($cookie[1]);
	  $lastname = trim($cookie[2]);
    } 
	else 
	{
    ...
    if ($db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_groups")) > 0) 
	{
      ...
      $db->sql_query("UPDATE ".$user_prefix."_users SET points=points+".$rpoints." WHERE firstname='".$firstname."' AND lastname='".$lastname."'");
    }
  }
}
...
function online() 
{
  ...
  $db->sql_query($sql);
  $sql = "SELECT time FROM ".$prefix."_session WHERE firstname='".addslashes($firstname)."' AND lastname='".addslashes($lastname)."'";
  $result = $db->sql_query($sql);
  $ctime = time();
  if (!empty($firstname) && !empty($lastname)) 
  {
    ...
    if ($row) 
	{
      $db->sql_query("UPDATE ".$prefix."_session SET firstname='".addslashes($firstname)."', lastname='".addslashes($lastname)."', time='".$ctime."', host_addr='".$ip."', guest='".$guest."' WHERE firstname='".addslashes($firstname)."' AND lastname='".addslashes($lastname)."'");
    } 
	else 
	{
      $db->sql_query("INSERT INTO ".$prefix."_session (firstname, lastname, time, host_addr, guest) VALUES ('".addslashes($firstname)."', '".addslashes($lastname)."', '".$ctime."', '".$ip."', '".$guest."')");
    }
  }
  $db->sql_freeresult($result);
}
...
function cookiedecode($user) 
{
    ...
    if (!isset($pass) && isset($cookie[1]) && isset($cookie[2])) 
	{
       $sql = "SELECT user_password FROM ".$user_prefix."_users WHERE firstname='".$cookie[1]."' AND lastname='".$cookie[2]."'";
       $result = $db->sql_query($sql);
       list($pass) = $db->sql_fetchrow($result);
       $db->sql_freeresult($result);
    }
    if (isset($cookie[3]) AND ($cookie[3] == $pass) AND (!empty($pass))) { return $cookie; }
}
...
function getusrinfo($user) 
{
	...
	{
        if ($userrow['firstname'] == $user[1] && $userrow['lastname'] == $user[2] && $userrow['user_password'] == $user[3]) 
		{
            return $userrow;
        }
    }
    $sql = "SELECT * FROM ".$user_prefix."_users WHERE firstname='".$user[1]."' AND lastname='".$user[2]."' AND user_password='".$user[3]."'";
    $result = $db->sql_query($sql);
...
//@todo aid
function get_author($aid, $aid2) 
{
	global $prefix, $db;
    static $users;
    if (isset($users[$aid]) && isset($users[$aid2]) && is_array($users[$aid]) && is_array($users[$aid2])) 
	{
        $row = $users[$aid];
		$row = $users[$aid2];
    } 
	...
	{
        $sql = "SELECT url, email FROM ".$prefix."_authors WHERE aid='".$aid."' AND aid2='".$aid2."'";
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $users[$aid] = $row;
		$users[$aid2] = $row;
	...
	{
        $aid = "<a href=\"".$aidurl."\">".$aid."&nbsp;".$aid2."</a>";
    } 
	elseif (isset($aidmail)) 
	{
        $aid = "<a href=\"mailto:".$aidmail."\">".$aid."&nbsp;".$aid2."</a>";
    } 
	else 
	{
        $aid = $aid . "&nbsp;" . $aid2;
...
//echo get_author($aid, $aid2);

function formatAidHeader($aid, $aid2) 
{
  $AidHeader = get_author($aid, $aid2);
  echo $AidHeader;
}
...
function adminblock() {
	...
	if (is_admin($admin)) 
	{
		...
		$content .= "<strong><big>&middot;</big></strong>&nbsp;<a href=\"".$admin_file.".php?op=submissions\">"._SUBMISSIONS."</a>: $num</span>";
...
function loginbox() {
	...
	if (!is_user($user)) {
		$title = _LOGIN;
		$boxstuff = "<form action=\"index.php?name=Your_Account\" method=\"post\">";
		$boxstuff .= "<center><font class=\"content\">"._FIRSTNAME."<br>";
		$boxstuff .= "<input type=\"text\" name=\"firstname\" size=\"8\" maxlength=\"31\"><br>";
		$boxstuff .= "<center><font class=\"content\">"._LASTNAME."<br>";
		$boxstuff .= "<input type=\"text\" name=\"lastname\" size=\"8\" maxlength=\"31\"><br>";
...
function userblock() {
  ...
  if(is_user($user)) {
    ...
    if($userinfo['ublockon']) {
      ...
      $title = _MENUFOR.'&nbsp;'.$cookie[1].'&nbsp;'.$cookie[2];
			...
			if (($date[4] < $hour) AND ($date[5] >= $min) OR ($date[4] <= $hour) AND ($date[5] <= $min)) {
				...
				while ($row2 = $db->sql_fetchrow($result2)) {
					...
					if ($num == 0) {
						...
						$db->sql_query("INSERT INTO ".$prefix."_stories VALUES (NULL, '".$catid2."', '".$aid2."', '".$aid3."', '".$title."', '".$time2."', '".$hometext."', '".$bodytext."', '0', '0', '".$topic2."', '".$firstname2."', '".$lastname2."', '".$notes."', '".$ihome2."', '".$alanguage2."', '".$acomm2."', '0', '0', '0', '0', '0', '".$associated2."')");
					}
				}
				$db->sql_freeresult($result2);
			}
...
function public_message() 
{
	...
	if ($broadcast_msg == 1) {
		if (is_user($user)) {
			cookiedecode($user);// @we here for username check
			$result = $db->sql_query("SELECT broadcast FROM ".$user_prefix."_users WHERE firstname='".$cookie[1]."' AND lastname='".$cookie[2]."'");
			$row = $db->sql_fetchrow($result);
			$upref = intval($row['broadcast']);
			if ($upref == 1) 
			{
				$t_off = '<br /><p align="right">[ <a href="index.php?name=Your_Account&amp;op=edithome">';
		...
		if (!is_user($user) || (is_user($user) && ($pm_show == 1))) 
		{
			$c_mid = base64_decode($p_msg);
			$c_mid = addslashes($c_mid);
			$c_mid = intval($c_mid);
			$result2 = $db->sql_query("SELECT mid, content, date, firstname, lastname FROM ".$prefix."_public_messages WHERE mid > '$c_mid' ORDER BY date ASC LIMIT 1");
			$row2 = $db->sql_fetchrow($result2);
			$mid = intval($row2['mid']);
			...
			if ((!isset($c_mid)) || ($c_mid = $mid)) 
			{
				...
				$public_msg .= '<font color="FFFFFF" size="3"><b>'._BROADCASTFROM.'&nbsp;<a href="index.php?name=Your_Account&amp;op=userinfo&amp;firstname='.$firstname.'&amp;lastname='.$lastname.'"><font color="FFFFFF" size="3">'.$firstname.'&nbsp;'.$lastname.'</font></a>: '.$content.'</b>';
				$public_msg .= $t_off;
...
function paid() 
{
	global $db, $user, $cookie, $adminmail, $sitename, $site_url, $subscription_url, $user_prefix, $prefix;
	if (is_user($user)) 
	{
		...
		elseif ($numrows != 0) 
		{
			...
			if ($row['subscription_expire'] <= $time) 
			{
				...
				$body = _HELLO.'&nbsp;'.$cookie[1].'&nbsp;'.$cookie[2].':<br /><br />'._SUBSCRIPTIONAT.'&nbsp;'.$sitename.'&nbsp;'._HASEXPIRED.'<br />'.$renew.'<br /><br />'._HOPESERVED.'<br /><br />'.$sitename.'&nbsp;'._TEAM.'<br />'.$site_url;
				$row = $db->sql_fetchrow($db->sql_query("SELECT user_email FROM ".$user_prefix."_users WHERE id='".$cookie[0]."' AND firstname='".$cookie[1]."' AND lastname='".$cookie[2]."' AND password='".$cookie[3]."'"));
				// Content Types
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/html; charset='._CHARSET.'' . "\r\n";
				// Additional headers
				$headers .= 'To: '.$cookie[1].' '.$cookie[2].' <'.$row['user_email'].'>' . "\r\n";
				$headers .= 'From: '.$sitename.' <'.$adminmail.'>' . "\r\n";
				// Mail Message
				mail($row['user_email'], $subject, $body, $headers);
			}
			return 1;
		}
	} else {
...
function ads($position) {
	global $prefix, $db, $admin, $sitename, $adminmail, $site_url;
...
function redir($content) {
	...
	for ($i=0; $i<sizeof($hrefs); $i++) 
	{
		...
		$content = str_replace('<a href="'.$hrefs[$i].'">', '<a href="'.$site_url.'/index.php?url='.$url.'" target="_blank">', $content);
	}
	return($content);
}
...
function info_box($graphic, $message) {
	...
	if (file_exists("images/system/".$graphic.".gif") && !empty($message)) {
		Open_Table();
