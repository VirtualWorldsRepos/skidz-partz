...
function defaultDisplay() {
	global $AllowableHTML, $prefix, $user, $cookie, $sl_firstname, $sl_lastname, $currentlang, $multilingual, $db, $module_name, $wysiwyg_editor;
	include ("header.php");
	Open_Table();
	...
	echo "<tr><td><form action=\"index.php?name=$module_name\" method=\"post\">"
		."<b>"._YOURNAME.":</b></td><td>";
	if (is_user($user)) {
		cookiedecode($user);
		echo "<a href=\"index.php?name=Your_Account\">".$cookie[1]."&nbsp;".$cookie[2]."</a> <font class=\"content\">[ <a href=\"index.php?name=Your_Account&amp;op=logout\">"._LOGOUT."</a> ]</font>";
	} else {
		echo "".$sl_firstname."&nbsp;".$sl_lastname."<font class=\"content\">[ <a href=\"index.php?name=Your_Account\">"._NEWUSER."</a> ]</font>";
	}
	echo "</td></tr>"
...
function PreviewStory($firstname, $lastname, $address, $subject, $story, $storyext, $topic, $alanguage) {
	global $user, $cookie, $bgcolor1, $bgcolor2, $sl_firstname, $sl_lastname, $prefix, $multilingual, $AllowableHTML, $db, $module_name, $wysiwyg_editor;
	include ("header.php");
...
function submitStory($firstname, $lastname, $address, $subject, $story, $storyext, $topic, $alanguage) {
	global $user, $EditedMessage, $cookie, $sl_firstname, $sl_lastname, $notify, $notify_email, $notify_subject, $notify_message, $notify_from, $prefix, $db;
	if (is_user($user)) {
		cookiedecode($user);
		$uid = $cookie[0];
		$firstname = $cookie[1];
	...
	if ($numrows == 0) {
		$result = $db->sql_query("INSERT INTO ".$prefix."_queue VALUES (NULL, '$uid', '$firstname', '$lastname', '$subject', '$story', '$storyext', now(), '$topic', '$alanguage')");
		if(!$result) {
			echo ""._ERROR."<br>";
			exit();
		}
...
switch($op) {
	...
	case ""._PREVIEW."":
	PreviewStory($firstname, $lastname, $address, $subject, $story, $storyext, $topic, $alanguage);
	break;

	case ""._OK."":
	SubmitStory($firstname, $lastname, $address, $subject, $story, $storyext, $topic, $alanguage);
	break;
